"""Ask the Missouri Public Data AI Assistant with controlled evidence tools."""
...
from missouri_public_data_ai.ag_market_news_index import AgMarketNewsIndex
from missouri_public_data_ai.ag_market_report_documents import AgMarketReportDocumentIndex
from missouri_public_data_ai.cannabis_index import CannabisIndex
from missouri_public_data_ai.child_care_index import ChildCareIndex
from missouri_public_data_ai.contract_documents import ContractDocumentIndex
from missouri_public_data_ai.contract_lookup import ContractIndex
from missouri_public_data_ai.data_mo_agriculture_index import DataMoAgricultureIndex
from missouri_public_data_ai.data_mo_catalog_index import DataMoCatalogIndex
from missouri_public_data_ai.data_mo_dnr_hazardous_waste_index import DataMoDnrHazardousWasteIndex
from missouri_public_data_ai.data_mo_dnr_oil_gas_index import DataMoDnrOilGasIndex
from missouri_public_data_ai.data_mo_education_index import DataMoEducationIndex
from missouri_public_data_ai.data_mo_farmers_market_index import DataMoFarmersMarketIndex
from missouri_public_data_ai.data_mo_food_pantry_index import DataMoFoodPantryIndex
from missouri_public_data_ai.data_mo_health_index import DataMoHealthIndex
from missouri_public_data_ai.data_mo_hospital_index import DataMoHospitalIndex
from missouri_public_data_ai.data_mo_ltc_index import DataMoLtcIndex
from missouri_public_data_ai.data_mo_utility_index import DataMoUtilityIndex
from missouri_public_data_ai.data_mo_water_index import DataMoWaterIndex
from missouri_public_data_ai.data_mo_wic_index import DataMoWicIndex
from missouri_public_data_ai.deep_answer import (
    EVIDENCE_TOOLS,
    DeepAnswerProvider,
    build_evidence_bundle,
    create_default_provider,
)
from missouri_public_data_ai.dese_apr_index import DeseAprIndex
from missouri_public_data_ai.dese_assessment_index import DeseAssessmentIndex
from missouri_public_data_ai.dese_directory_index import DeseDirectoryIndex
from missouri_public_data_ai.dese_finance_index import DeseFinanceIndex
from missouri_public_data_ai.dese_school_data_index import DeseSchoolDataIndex
from missouri_public_data_ai.dese_special_education_index import DeseSpecialEducationIndex
from missouri_public_data_ai.dhss_brfss_index import DhssBrfssIndex
from missouri_public_data_ai.dhss_health_sources_index import DhssHealthSourcesIndex
from missouri_public_data_ai.dhss_ltc_inspection_index import DhssLtcInspectionIndex
from missouri_public_data_ai.dhss_mophims_profiles_index import DhssMophimsProfilesIndex
from missouri_public_data_ai.dhss_vital_stats_index import DhssVitalStatsIndex
from missouri_public_data_ai.dnr_impaired_waters_index import DnrImpairedWatersIndex
from missouri_public_data_ai.dnr_resources_index import DnrResourcesIndex
from missouri_public_data_ai.dor_reports_index import DorReportsIndex
from missouri_public_data_ai.evidence_coordinator import EvidenceCoordinator, wants_multi_source
from missouri_public_data_ai.expanded_public_sources import PublicSourceIndex
from missouri_public_data_ai.map_public_index import MapPublicIndex, normalize_public_name, years_in_question
from missouri_public_data_ai.mec_annual_report_index import MecAnnualReportIndex
from missouri_public_data_ai.mec_resources_index import MecResourcesIndex
from missouri_public_data_ai.meric_labor_index import MericLaborIndex
from missouri_public_data_ai.modot_aadt_index import ModotAadtIndex
from missouri_public_data_ai.mshp_crash_index import MshpCrashIndex
from missouri_public_data_ai.msdis_geospatial_index import MsdisGeospatialIndex
from missouri_public_data_ai.oa_budget_index import OaBudgetIndex
from missouri_public_data_ai.oa_revenue_detail_index import OaRevenueDetailIndex
from missouri_public_data_ai.psc_report_documents import PscReportDocumentIndex
from missouri_public_data_ai.psc_reports_index import PscReportsIndex
from missouri_public_data_ai.public_source_catalog import PUBLIC_SOURCE_CATALOG, catalog_by_status
from missouri_public_data_ai.sos_elections_index import SosElectionsIndex
from missouri_public_data_ai.state_auditor_documents import StateAuditorDocumentIndex
from missouri_public_data_ai.state_auditor_index import StateAuditorIndex


PROJECT_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_MODEL = "HuggingFaceTB/SmolLM2-135M-Instruct"
DEFAULT_ADAPTER = PROJECT_ROOT / "checkpoints" / "smollm2_135m_lora_run_002"
DEFAULT_DEEP_ANSWER_MODE = "default"
...
SOURCE_METADATA = {
    ...
    "generalized_evidence_index": ("Generalized evidence registry", "document/row evidence lookup"),
...
ROUTE_SPECIFICATIONS = [
    ...
    ("map_public_lookup_index", "Missouri Accountability Portal", "exact public-record lookup", "asks_about_expenditure_lookup", ["map", "expenditure", "expenditure total", "spending"]),
    ...
    ("missouri_contract_document_lookup_index", "Missouri contract documents", "document text lookup", "asks_for_contract_document_snippet", ["document text", "renewal", "language", "snippet"]),
    ("missouri_contract_document_lookup_index", "Missouri contract documents", "document text lookup", "asks_about_contract_document_index", ["document text", "documents indexed", "pdf text"]),
    ...
    ("missouri_public_source_catalog", "Missouri public source catalog", "source discovery", "asks_about_public_source_catalog", ["public data sources", "source catalog", "add next"]),
    ...
    ("dhss_health_sources_lookup_index", "DHSS health resources", "source discovery", "asks_about_dhss_health_sources_lookup", ["dhss health", "brfss link", "mica", "focus report", "resources"]),
    ...
    ("data_mo_health_lookup_index", "data.mo.gov public health", "exact aggregate lookup", "asks_about_health_lookup", ["communicable disease", "anaplasmosis", "current week ytd", "ytd", "disease report"]),
    ...
    ("mec_resources_lookup_index", "MEC resources", "source discovery", "asks_about_mec_resources_lookup", ["mec", "resource", "campaign finance", "reports", "connected"]),
    ("mec_annual_report_lookup_index", "MEC annual reports", "exact aggregate lookup", "asks_about_mec_annual_report_lookup", ["mec annual report", "lobbyists", "receipts"]),
...
CONTRACT_DOCUMENT_SNIPPET_PATTERNS = [
    ...
    r"\b(?:supposed|required|requires?|shall|must|responsible|restock|re-stock|provide|kitchenett\w*|break\s*room|breakroom)\b.*\bcontract\b",
    r"\bcontract\b.*\b(?:supposed|required|requires?|shall|must|responsible|restock|re-stock|provide|kitchenett\w*|break\s*room|breakroom)\b",
...
def canned_general_answer_text(question: str) -> str | None:
    ...
    if re.search(r"\bwhat\s+is\s+an?\s+assistant\b", normalized):
        return "An assistant is software that helps answer questions or complete tasks using rules, retrieved information, a language model, or a mix of those methods."
    ...
    if any(
        ...
        return (
            "This project builds the Missouri Public Data AI Assistant. "
            "It retrieves cited Missouri public-data evidence through local tools and can use Vertex AI Gemini to synthesize grounded answers."
        )
...
class AskEngine:
    def __init__(
        ...
        deep_answer_mode: str = DEFAULT_DEEP_ANSWER_MODE,
        deep_answer_provider: DeepAnswerProvider | None = None,
    ) -> None:
        ...
        self.deep_answer_mode = deep_answer_mode
        self.deep_answer_provider = deep_answer_provider or create_default_provider()
        ...
        self.evidence_coordinator = EvidenceCoordinator(self)
    ...
    def deep_answer_enabled(self) -> bool:
        return self.deep_answer_mode.lower() not in {"off", "none", "disabled"}

    def provider_name(self) -> str:
        return str(getattr(self.deep_answer_provider, "name", "unknown"))

    def provider_model_id(self) -> str:
        return str(getattr(self.deep_answer_provider, "model_id", "unknown"))

    def append_source_note(self, result: dict[str, Any], note: str) -> dict[str, Any]:
        updated = dict(result)
        existing = str(updated.get("source_note") or "").strip()
        updated["source_note"] = f"{existing} {note}".strip()
        return updated

    def local_verification_summary(self, result: dict[str, Any]) -> dict[str, Any]:
        model = str(result.get("model") or "")
        return {
            "privacy_guardrail_checked": True,
            "unsupported_scope_checked": True,
            "citation_count": len(result.get("citations") or []),
            "source_row_count": len(result.get("source_rows") or []),
            "evidence_hit_count": len(result.get("evidence_hits") or []),
            "evidence_bundle_id": result.get("evidence_bundle_id"),
            "guardrail_reason": result.get("guardrail_reason"),
            "refusal_preserved": model in {"public_data_boundary", "unsupported_scope_guardrail", "retrieval_guardrail"},
        }

    def apply_deep_answer(self, question: str, result: dict[str, Any]) -> dict[str, Any]:
        if not self.deep_answer_enabled():
            return result

        bundle = build_evidence_bundle(question, result)
        updated = dict(result)
        updated["assistant_name"] = "Missouri Public Data AI Assistant"
        updated["deep_answer_provider"] = self.provider_name()
        updated["deep_answer_model"] = self.provider_model_id()
        updated["deep_answer_mode"] = self.deep_answer_mode
        updated["available_evidence_tools"] = [tool["name"] for tool in EVIDENCE_TOOLS]
        updated["evidence_tool_calls"] = bundle["tool_calls"]
        updated["evidence_hits"] = result.get("evidence_hits", bundle.get("evidence_hits", []))
        updated["evidence_bundle_id"] = result.get("evidence_bundle_id") or bundle.get("evidence_bundle_id")
        updated["raw_evidence_summary"] = result.get("raw_evidence_summary") or bundle.get("raw_evidence_summary")
        updated["confidence"] = result.get("routing_confidence", result.get("retrieval_score", 0.0))
        updated["limitations"] = bundle["limitations"]
        updated["local_verification"] = self.local_verification_summary(result)

        if result.get("model") in {"public_data_boundary", "unsupported_scope_guardrail", "retrieval_guardrail"}:
            updated["deep_answer_status"] = "guardrail_preserved"
            return updated

        if result.get("model") == "general_chat":
            updated["deep_answer_status"] = "not_applicable"
            return updated

        if not self.deep_answer_provider.is_available():
            updated["deep_answer_status"] = "unavailable_fallback"
            return self.append_source_note(
                updated,
                "Deep Answer Mode is configured for Vertex AI, but the provider is unavailable; returned local evidence output.",
            )

        try:
            generated = self.deep_answer_provider.generate(question, bundle)
        except Exception as exc:
            updated["deep_answer_status"] = "provider_error_fallback"
            updated["deep_answer_error"] = str(exc)
            return self.append_source_note(
                updated,
                "Deep Answer Mode attempted cloud synthesis but fell back to local evidence output.",
            )

        answer = str(generated.get("answer") or "").strip()
        if not answer:
            updated["deep_answer_status"] = "empty_provider_fallback"
            return self.append_source_note(updated, "Deep Answer Mode returned no synthesis; kept local evidence output.")

        updated["raw_evidence_answer"] = str(result.get("answer") or "")
        updated["answer"] = answer
        updated["used_model"] = True
        updated["synthesis_model"] = self.provider_model_id()
        updated["synthesis_mode"] = "deep_answer"
        updated["deep_answer_status"] = "synthesized"
        updated["confidence"] = generated.get("confidence", updated["confidence"])
        updated["limitations"] = generated.get("limitations", updated["limitations"])
        updated["provider_metadata"] = generated.get("provider_metadata", {})
        return self.append_source_note(
            updated,
            "Deep Answer Mode used cloud synthesis over controlled local Missouri evidence.",
        )
    ...
    def synthesize_answer(self, question: str, result: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            prompt = (
                "You are Missouri Public Data AI Assistant.\n"
    ...
    def general_model_answer_text(self, question: str) -> str:
        tokenizer, model, device = self.ensure_model()
        prompt = (
            "You are a concise assistant inside a Missouri public-data demo.\n"
    ...
    def general_chat_answer(self, question: str) -> dict[str, Any] | None:
        ...
        if any(phrase in lowered for phrase in ["who are you", "what are you", "what can you do"]):
            return {
                "question": question,
                "answer": (
                    "I am the Missouri Public Data AI Assistant. I retrieve local Missouri public-data evidence, "
                    "show source links for public-data answers, and use Deep Answer Mode when a provider is configured."
                ),
                ...
                "used_model": False,
                "model": "general_chat",
                "source_note": "No external source used for this general assistant description.",
                "citations": [],
                "source_rows": [],
            }
        ...
        result: dict[str, Any] = {
            "question": question,
            "answer": (
                "I do not have a local evidence-backed answer for that. Ask a Missouri public-data question, "
                "or configure Deep Answer Mode for broader cloud reasoning."
            ),
            "retrieved_context_id": "general_chat:deep_answer_required",
            "retrieved_source": None,
            "retrieval_score": 1.0,
            "used_model": False,
            "model": "general_chat",
            "source_note": "No local language model was used; broad general answers require the configured deep-answer provider.",
            "citations": [],
            "source_rows": [],
        }
        return result
    ...
    def public_source_catalog_answer(self, question: str) -> dict[str, Any]:
        ...
        lines = [
            "Best public-data expansion targets for this assistant:",
            "",
            "Already connected:",
        ]
        ...
        lines.append("")
        lines.append(
            "Product direction: prioritize contracts first because the assistant can combine contract metadata, document links, "
    ...
    def contract_document_query_terms(self, question: str) -> list[str]:
        ...
        if "kitchen" in lowered:
            terms.append("kitchenette")
        if "break room" in lowered or "breakroom" in lowered:
            terms.append("break room")
        if "restock" in lowered or "re-stock" in lowered:
            terms.append("restock")
            terms.append("dispensers")
        if "paper towel" in lowered:
            terms.append("paper towels")
        if "soap" in lowered:
            terms.append("soap")
        if terms:
    ...
    def contract_document_citations(self, document: dict[str, Any]) -> list[dict[str, Any]]:
        url = document.get("url") or document.get("detail_url") or ""
        return [
            {
                "dataset": "Missouri public contract documents",
                "category": "Missouri Contracts",
                "kind": "contract document text lookup",
                "lookup_table": "local_and_selected_contract_document_index",
                "year": None,
                "year_range": None,
                "source_files": [
                    {
                        "category": "contracts",
                        "category_label": document.get("label", "Official contract document"),
                        "file_name": url,
                        "source_url": url,
                        "row_count": None,
    ...
    def selected_contract_document_answer(self, question: str, document: dict[str, Any]) -> dict[str, Any]:
        terms = self.contract_document_query_terms(question)
        snippets = self.contract_document_index.snippets_for_document(document, terms)
        lowered = question.lower()
        asks_kitchenette = "kitchen" in lowered or "break room" in lowered or "breakroom" in lowered
        asks_restock = "restock" in lowered or "re-stock" in lowered or "supposed" in lowered or "required" in lowered
        if asks_kitchenette and asks_restock:
            answer = (
                "No, not by default. The Fletcher Daniels janitorial RFQ says break room/kitchenette products are "
                "not required unless another requirement specifically says otherwise. It separately requires restroom "
                "dispensers to be restocked to normal limits, including soap, toilet tissue, paper towels, and toilet "
                "seat covers. So the evidence supports restroom dispenser restocking, not general kitchenette restocking."
            )
        else:
            snippet_text = " | ".join(snippets[:3]) if snippets else clean_answer_text(str(document.get("text") or "")[:500])
            answer = (
                f"Contract document text for {document.get('contract_number')} ({document.get('description')}): "
                f"{snippet_text} Source document: {document.get('url')}."
            )
        return {
            "question": question,
            "answer": answer,
            "retrieved_context_id": f"missouri_contract_documents:selected:{document.get('contract_number')}",
            "retrieved_source": "missouri_contract_document_index",
            "retrieval_score": 1.0,
            "used_model": False,
            "model": "deterministic_public_lookup",
            "source_note": (
                "Answer from selected official OA contract/RFQ document text. Use the official linked document "
                "for full legal terms."
            ),
            "citations": self.contract_document_citations(document),
            "source_rows": [
                {
    ...
    def contract_answer(self, question: str) -> dict[str, Any]:
        ...
        if contract is None:
            if asks_for_contract_document_snippet(question):
                document_matches = self.contract_document_index.search_documents(
                    question, limit=requested_limit(question, default=3, maximum=5)
                )
                if document_matches:
                    return self.selected_contract_document_answer(question, document_matches[0])
    ...
    def route_question(self, question: str) -> dict[str, Any]:
        ...
        if asks_unsupported_scope(question):
            return {
                "question": question,
                "answer": (
                    "I do not have indexed source support for that request. This assistant does not forecast, verify active contracts or endorsements, "
        ...
        if general_result is not None and general_result.get("retrieved_context_id") not in {
            "general_chat:local",
            "general_chat:deep_answer_required",
        }:
            return general_result

        if wants_multi_source(question):
            evidence_result = self.evidence_coordinator.generic_answer(question)
    ...
    def ask(self, question: str) -> dict[str, Any]:
        ...
        result = self.evidence_coordinator.enrich_result(question, result)
        result = self.apply_deep_answer(question, result)
...
def main() -> None:
    ...
    result = ask(
        ...
        deep_answer_mode=args.deep_answer,
    )
