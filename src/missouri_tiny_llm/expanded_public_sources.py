...
SOURCES: tuple[SourceSpec, ...] = (
    ...
    SourceSpec(
        ...
        useful_for="official election source discovery plus selected exact statewide election-return lookup for winners, candidate votes, percentages, total votes, and primary party winners",
...
DEDICATED_PARSER_NOTES = {
    ...
    "dhss": (
        ...
        "selected DHSS MOPHIMS statewide ProfileBuilder rows, selected county inpatient-hospitalization rows, and DHSS WIC county/municipality aggregates; broader MOPHIMS/MICA query values, county-level BRFSS values, "
        "county-level births/deaths, and broader hospitalizations still need separate aggregate parsers."
    ),
    "long_term_care": (
    ...
    "agriculture": (
        ...
        "selected Agricultural Market News report-link metadata, and capped selected Agricultural Market News PDF text/value rows for Missouri hay and Joplin feeder-cattle reports; broader market-report PDFs, seed samples, inspections, complaints, and enforcement sources still need separate parsers."
    ),
}
