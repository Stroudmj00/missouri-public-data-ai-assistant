...
def write_model_card(report: dict[str, Any]) -> None:
    content = f"""# Model Card

## Current Model State

This repository has not trained a new model yet. The current artifact is a first baseline inference run using `{report['model_id']}`.

## Baseline Purpose

The baseline tests whether a tiny instruct model can answer simple questions when each prompt includes a short, sanitized public-data context snippet.
...
Educational case study and reproducibility scaffold for a future tiny-LLM fine-tuning experiment.
...
The model has not yet been trained on the generated QA set. The baseline should be interpreted as a starting point for comparison, not a finished assistant.
"""
