...
def fetch_text(url: str) -> str:
    response = requests.get(url, timeout=30, headers={"User-Agent": "missouri-tiny-llm-case-study/1.0"})
...
def build_dnr_impaired_waters_index(
    ...
    response = requests.get(pdf_url, timeout=60, headers={"User-Agent": "missouri-tiny-llm-case-study/1.0"})
    response.raise_for_status()
