# Missouri Public Data AI Assistant Capability Upgrade

## Goal

Move the project from a narrow demo toward a genuinely useful Missouri Public Data AI Assistant.

## Implemented

- Fail-closed fallback: unsupported or low-source-support questions do not call model synthesis by default.
- Deep Answer Mode: the assistant now has a provider layer for Vertex AI Gemini synthesis over controlled local evidence-tool output, with fake and unavailable providers for tests.
...
- Common routing diagnostics: every answer now exposes `retrieval_path`, `source_family`, `evidence_type`, `routing_confidence`, optional `guardrail_reason`, top ranked `route_candidates`, evidence-tool calls, evidence hits, evidence bundle ids, provider status, confidence, limitations, and local verification metadata for reviewer-facing audits.
- Assistant behavior regression script: `scripts/test_assistant_behavior.py` with 330 expanded exact-lookup, general-chat, civic-fact, hyperlink, hospital-profile, contract payment-context, generalized evidence registry, manifest-only source, multi-hit Boone County briefing, contract document text/snippet, selected contract RFQ clause lookup, DESE assessment, DESE staff, food pantry, farmers-market, DNR oil-and-gas permit, DNR hazardous-waste facility, SOS county/turnout, MSHP county crash, citizen-definition, DHSS county vital-statistics, routing, deep-answer provider fallback, fake provider synthesis, comparison, ranking, unsupported complex, and privacy-sensitive complex cases.
