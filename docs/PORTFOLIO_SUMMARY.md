...
Built a reproducible tiny-LLM case study using public Missouri finance, health, DHSS public-health resource, education, elections, utility, PSC report, OA Budget, agriculture, cannabis-regulation, child-care, and school-data resource datasets, with a CUDA-enabled local training environment, aggregate QA generation, baseline evaluation, LoRA fine-tuning, deterministic lookup for indexed public records, and documented data boundaries.
...
- Deterministic lookup for exact indexed public records such as MAP totals, contracts, election returns, utility providers, DESE School Data resource links, DHSS public-health resource links, PSC report metadata, OA Budget metadata, Agricultural Market News report links, and selected cannabis-regulation facts
