...
The runtime tests now report behavior by category instead of only a single pass count. Each API answer carries a common routing shape:
...
This keeps the case study focused on source-grounded retrieval and deterministic lookup. Local LLM synthesis remains optional and subordinate to cited lookup output.
